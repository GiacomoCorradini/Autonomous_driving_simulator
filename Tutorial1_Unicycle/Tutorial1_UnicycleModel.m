...
Qi = [3 3 pi/3];    % initail condition [x, y, theta]
Qf = [0 0 0];       % final condition [x, y, theta]
...
[vec_ud,x,y,theta] = ClotoidPath(Tmin, Tmax, Tsample, Qi, Qf);
...
%     sample = sqrt((Qf(1)-Qi(1))^2+(Qf(2)-Qi(2))^2)/Tsample;
%     x = Qf(1):0.1:Qi(1);
%     y = Qf(2):0.1:Qi(2);
%     figure, hold on, grid on
%     plot(x,y,'.')
...
for i = 1:length(vec_ud)-2
    ...
    %Better...
    x_dot(i)     = cos(theta(i))*vec_ud(i,1);
    y_dot(i)     = sin(theta(i))*vec_ud(i,1);
